...
export function TableOfContents({ content, scrollContainerId, onItemClick }: TableOfContentsProps) {
  ...
  return (
    <nav className="py-4 px-2">
      <div className="text-xs font-orbitron font-bold text-cyber-cyan mb-3 px-2">
        目录
      </div>
          ...
          return (
            <li key={item.id}>
              <button
                ...
                className={`w-full text-left text-sm py-2 px-2 rounded transition-all flex items-start gap-2 group ${
                  isActive
                    ? 'bg-cyber-cyan/20 text-cyber-cyan border-l-2 border-cyber-cyan'
                    : 'text-white/60 hover:text-white hover:bg-white/5 border-l-2 border-transparent'
                }`}
                style={{ paddingLeft: `${paddingLeft}px` }}
              >
                <ChevronRight 
                  className={`w-3 h-3 flex-shrink-0 mt-0.5 transition-transform ${
