...
export function QuotaDisplay({ agentKey, className = "", showLabel = true }: QuotaDisplayProps) {
  ...
  const getStatusIcon = () => {
    if (quota.remaining === 999) return <Zap className="h-4 w-4" />;
    if (quota.remaining > 2) return <CheckCircle className="h-4 w-4" />;
  ...
  return (
    <div className={`flex items-center gap-2 ${className}`}>
      {showLabel && <span className="font-mono text-xs text-gray-500">今日配额</span>}
