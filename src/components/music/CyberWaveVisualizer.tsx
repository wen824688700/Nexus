...
export const CyberWaveVisualizer = ({ isPlaying, getFrequencyData }: CyberWaveVisualizerProps) => {
    ...
    const drawWave = () => {
      ...
      for (let wave = 0; wave < waveCount; wave++) {
        ...
        for (let x = 0; x <= canvas.width; x += 2) {
          ...
          const y = yOffset + 
            Math.sin(x * frequency + phase + wave * 0.5) * amplitude * (0.5 + audioInfluence * 0.5) +
  ...
  return (
    <div className="fixed bottom-0 left-0 right-0 h-[200px] pointer-events-none z-0 overflow-hidden">
      {/* 背景渐变 */}
      <div 
