...
export function handleAuthError(error: unknown): AuthError {
  ...
  if (error instanceof SupabaseAuthError) {
    ...
    return new AuthError(message, error.message, error.status, error);
  }

  // 处理网络错误
  if (error instanceof TypeError && error.message.includes("fetch")) {
    return new AuthError(ERROR_MESSAGES["network_error"], "network_error", undefined, error);
  }

  // 处理超时错误
  if (error instanceof Error && error.message.includes("timeout")) {
    return new AuthError(ERROR_MESSAGES["timeout"], "timeout", undefined, error);
  }
  ...
  if (typeof error === "object" && error !== null) {
    ...
    if (errorObj.status === 429 || errorObj.statusCode === 429) {
      return new AuthError(ERROR_MESSAGES["too_many_requests"], "too_many_requests", 429, error);
    }
  ...
  if (error instanceof Error) {
    ...
    return new AuthError("认证失败，请重试", "unknown_error", undefined, error);
  }

  // 未知错误类型
  return new AuthError("发生未知错误，请重试", "unknown_error", undefined, error);
}
...
export function isSessionExpiredError(error: unknown): boolean {
  ...
  if (typeof error === "object" && error !== null) {
    ...
    return (
      errorObj.message?.includes("session") === true ||
